...
generateParams <- function() {
  ...
  return(
    list(
      alpha.v = alpha.v,
      beta.m = beta.m,
      perturb.s = perturb$start,
      perturb.e = perturb$end,
      gamma = gamma.v
    )
  )
}
generateSimultatedDataset <- function(counts, err.to.equil) {
  counts <- NULL
  count <- 0
  ...
  for (i in 1:(1000 * n)) {
    ...
    while (TRUE) {
      ...
      if (all(abs(out[(nrow(out)), -1] - out[(nrow(out) - 1), -1]) < 1e-5))
        break
    ...
    if (count > samples_at_equil) {
      ...
      tmp <-
        abs(t(out[, -1]) - as.numeric(end)) / as.numeric(end) ## relative error
      idx <-
        which(apply(tmp > thres_to_eq[1], 2, function(x)
          sum(x, na.rm = TRUE) / sum(!is.na(x))) > thres_to_eq[2])
    ...
    tmp[is.infinite(tmp)] <-
      NA ## num/0 --> max error !! Not reaching 0 yet
    ...
    if (count >= n)
      break ## generate n replicates
  }
  return(list(counts = counts, err.to.equil = err.to.equil))
}
# simultated_dataset <- generateSimultatedDataset()
rerun <- TRUE
while(rerun) {
  print("Generating model parameters...")
  parms <- generateParams()
  print("Generating simulated dataset...")
  simultated_dataset <- try(generateSimultatedDataset(), TRUE)
  
  if (class(simultated_dataset) == 'try-error') {
    rerun <- TRUE
    print('error')
  } else {
    rerun <- FALSE
...
counts <- simultated_dataset$counts
err.to.equil <- simultated_dataset$err.to.equil
...
write.table(
  counts,
  paste0(out.folder, "/", PREFIX, '.n', n, '_d', p, ".counts.txt"),
  col.names = F,
  row.names = T,
  quote = F,
  sep = "\t"
...
write.table(
  t(err.to.equil),
  paste0(out.folder, "/", PREFIX, '.n', n, '_d', p, ".err_to_equil.txt"),
  col.names = F,
  row.names = F,
  quote = F,
  sep = "\t"
...
## write.table(biomass,  paste0(out.folder,"/",PREFIX,'.n',n,'_d', p, ".biomass.txt"), col.names = T, row.names = F, quote =F, sep = "\t")

## write paramters
parameters.out <- parms$beta.m
rownames(parameters.out) <-
  colnames(parameters.out) <- sprintf("sp_%03d", 1:p)
parameters.out <- melt(parameters.out)
parameters.out <-
  rbind(data.frame(
    Var1 = sprintf("sp_%03d", 1:p),
    Var2 = NA,
    value = parms$alpha.v
  ),
  parameters.out)[, c(2, 1, 3)]

colnames(parameters.out)[1:2] <- c('source_taxon', 'target_taxon')

write.table(
  parameters.out,
  paste0(out.folder, '/', PREFIX, '.n', n, '_d', p, ".truth.txt"),
  quote = F,
  col.names = T,
  row.names = F ,
  sep = '\t'
