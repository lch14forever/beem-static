...
base <- '/mnt/projects/lich/dream_challenge/BEEM_STATIC/simulation/test_p20_n500_thre0.2_thre0.5/'

df.noNoise <- read.table(paste0(base,'/counts.txt'), head=F, row.names=1, sep='\t')

dist.to.eq <- read.table(paste0(base,'/err_to_equil.txt'), head=F, sep='\t')
...
truth <- read.table(paste0(base,'/truth.txt'),head=T)
...
res <- func.EM(dat, ncpu=ncpu, scaling=scaling, dev=Inf, max.iter=max.iter)
...
tmp <- cmdscale(proxy::dist(log(abs(err)+1), method='cosine'))
